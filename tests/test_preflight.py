...
def test_preflight_geoblock_pass_when_blocked_but_clob_auth_ok_in_shadow(monkeypatch):
    ...
    assert "egress OK" in geo.detail
...
def test_preflight_geoblock_fail_when_blocked_but_clob_auth_ok_in_live(monkeypatch):
    ...
    monkeypatch.setattr(
        "world_cup_bot.clob_rest.probe_order_post_status",
        lambda *_a, **_k: 403,
    )
...
def test_preflight_geoblock_pass_when_blocked_but_post_probe_ok_in_live(monkeypatch):
    monkeypatch.setenv("DRY_RUN", "false")
    monkeypatch.setenv("POLYMARKET_PRIVATE_KEY", "0x" + "11" * 32)
    monkeypatch.setenv("POLYMARKET_POLY_ADDRESS", "0x" + "22" * 20)
